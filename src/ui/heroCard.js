...
import { getHeroIcon } from './heroIcon.js';
import { syncHeroTokenVisual } from '../animation/heroToken.js';

export function setReaction(text) {
...
export function updateHeroCardVisual(hero) {
...
export function updateHeroCard(hero) {
  var card = document.getElementById('hero-card');
  if (!card) return;
...
export function flashSlot(slotEl, kind) {
  if (!slotEl) return;
  var cls =
    kind === 'kill'
      ? 'slot-kill'
